...
export default function OnDirectQuestionsSetup() {
    ...
    const [all, setAll] = useState<Questions[]>([]);
    ...
    const getAllHandler = () => {
        setLoading(true);
        fetch(`${url}Settings/GetOnDirectQuestions?pageSize=${paging.pageSize}&pageNumber=${paging.pageNumber}`, {
            ...
            response.json().then((result: PagedResponse<Array<Questions>>) => {
    ...
    const importFile = (file: any) => {
        ...
        setLoading(true);
        fetch(`${url}Settings/ImportOnDirectQuestions`, {
            method: 'POST',
            body: formData,
            headers: {
                'Authorization': `Bearer ${auth?.accessToken?.token}`
            ...
            <div className={'flex flex-col w-full h-full space-y-4'}>
                ...
                <div className={'grow w-full h-full overflow-auto'}>
                    <table className={'border-collapse w-full text-sm'}>
                        <thead>
                        <tr className="noWrap">
                            <th className={'border border-slate-600'}>QUESTIONS</th>
                            ...
                            <>
                                {
                                    all.map((data) =>
                                        <tr key={data.id}>
                                            <td className={'border-slate-700 border p-1'}> {data.question}</td>
