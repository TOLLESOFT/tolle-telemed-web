...
import {HistoryType} from "../../../shared/models/history-type";
...
export default function ExamCategoriesSetup() {
    ...
    const [all, setAll] = useState<ExamCategories[]>([]);
    ...
    const getAllHandler = () => {
        setLoading(true);
        fetch(`${url}Settings/GetExamCategories`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${auth?.accessToken?.token}`
            }
        }).then((response) => {
            response.json().then((result: ApiResponse<Array<ExamCategories>>) => {
                setAll(result.data);
            }).finally(() => {
                setLoading(false);
            });
    ...
    const importFile = (file: any) => {
        ...
        setLoading(true);
        fetch(`${url}Settings/ImportExamCategories`, {
            method: 'POST',
            body: formData,
            headers: {
                'Authorization': `Bearer ${auth?.accessToken?.token}`
            ...
            <div className={'flex flex-col w-full h-full space-y-4'}>
                ...
                <div className={'grow w-full h-full overflow-auto'}>
                    <table className={'border-collapse w-full text-sm'}>
                        <thead>
                        <tr className="noWrap">
                            <th className={'border border-slate-600'}>EXAM CATEGORIES</th>
                            ...
                            <>
                                {
                                    all.map((data) =>
                                        <tr key={data.id}>
                                            <td className={'border-slate-700 border p-1'}> {data.name} </td>
