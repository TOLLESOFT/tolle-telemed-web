...
export default function MedicalHistorySetup() {
    ...
    const [all, setAll] = useState<HistoryType[]>([]);
    ...
    const getAllHandler = () => {
        setLoading(true);
        fetch(`${url}Settings/GetMedicalHistory`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${auth?.accessToken?.token}`
            }
        }).then((response) => {
            response.json().then((result: ApiResponse<Array<HistoryType>>) => {
                setAll(result.data);
            }).finally(() => {
                setLoading(false);
            });
    ...
    const importFile = (file: any) => {
        ...
        setLoading(true);
        fetch(`${url}Settings/ImportMedicalHistory`, {
            method: 'POST',
            body: formData,
            headers: {
                'Authorization': `Bearer ${auth?.accessToken?.token}`
            ...
            <div className={'flex flex-col w-full h-full space-y-4'}>
                ...
                <div className={'grow w-full h-full overflow-auto'}>
                    <table className={'border-collapse w-full text-sm'}>
                        <thead>
                        <tr className="noWrap">
                            <th className={'border border-slate-600'}>HISTORY</th>
